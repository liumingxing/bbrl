...
var url = Ti.App.mamashai + "/api/account/taobao_mobile?from=baby_calendar";
if (win.id){
	url += "&id=" + win.id
}
...
webview.addEventListener('load',function(e) {
    if (!Ti.App.is_android){
	    ...
	    if (info && info.length > 0)
	    {
			...
			Ti.App.Properties.setString("login_type", "3");		//0:主站, 1:新浪 2:腾讯, 3: 淘宝
			Ti.App.Properties.setString("taobao_token", json.token);
			Ti.App.Properties.setString("taobao_secret", json.secret);
			Ti.App.Properties.setString("taobao_expire_at", json.expire_at);
			Ti.App.Properties.setString("token_type", "taobao");
			
	    	require('lib/mamashai_db').db.insert_json("user_profile", json.user.id, JSON.stringify(json.user))
			Titanium.UI.currentWindow.close();
			Ti.App.fireEvent("taobao_login");
	    }
    }
    else{
    	Ti.API.log(e.url)
    	if (e.url.indexOf("blank?")>0){
			...
			Ti.App.Properties.setString("login_type", "3");		//0:主站, 1:新浪 2:腾讯
			Ti.App.Properties.setString("taobao_token", params.token);
			Ti.App.Properties.setString("taobao_secret", '');
			Ti.App.Properties.setString("token_type", "taobao");
			Ti.App.Properties.setString("taobao_expire_at", params.expire_at);
