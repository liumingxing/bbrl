...
var url = Ti.App.mamashai + "/api/account/tencent_mobile?from=baby_calendar";
if (win.id){
	url += "&id=" + win.id
}
...
webview.addEventListener('load',function(e) {
    if (!Ti.App.is_android){
	    ...
	    if (info.length > 0)
	    {
			...
			Ti.App.Properties.setString("login_type", "2");		//0:主站, 1:新浪 2:腾讯
			Ti.App.Properties.setString("tencent_token", json.token);
			Ti.App.Properties.setString("tencent_secret", json.secret);
			Ti.App.Properties.setString("token_type", "tencent");
			require('lib/mamashai_db').db.insert_json("user_profile", json.user.id, JSON.stringify(json.user))
			Titanium.UI.currentWindow.close();
			Ti.App.fireEvent("tencent_login");
	    }
    }
    else{
    	if (e.url.indexOf("blank?")>0){
			...
			Ti.App.Properties.setString("login_type", "2");		//0:主站, 1:新浪 2:腾讯
			Ti.App.Properties.setString("tencent_token", params.token);
			Ti.App.Properties.setString("tencent_secret", '');
			Ti.App.Properties.setString("token_type", "tencent");
			Ti.App.Properties.setString("expire_at", params.expire_at);
