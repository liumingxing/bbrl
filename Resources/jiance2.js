function Jiance(attr){
	...
	win.addEventListener("open", function(e){
		...
		for(var i = 0; i <= 35; i++) {
			data.push({
				text : (parseInt(i/12) > 0 ? parseInt(i/12) + '岁' : (i==0 ? '新生' : '')) + (i%12==0 ? '' : i%12 + "个月"),
				value : i
			})
		}
		...
		week_container.addEventListener("tab_click", function(e){
				var record = Ti.App.db2.execute('SELECT * FROM fayus where id=?', e.value+1);
				if(record.isValidRow()) {
					...
					if (Ti.App.is_bbrl){
						ZhinanAdv.zhinan_adv('0_3', wrapper1)
					}	
					
					if (!Ti.App.is_android)
						pregant_content.addEventListener("swipe", swipe);
					
					var pic_1 = Ti.UI.createImageView({
							top : __l(4),
							hires : true,
							//width : blob ? __l(blob.width/2) : __l(240),
							//height : blob ? __l(blob.height/2) : __l(200),
							left: __l(40),
							right: __l(40),
							image : Ti.App.aliyun + "/jiance/" + (e.value+1) + ".jpg"
					});
					pregant_content.add(pic_1);
					...
					if (!Ti.App.is_bbrl)
						ZhinanAdv.zhinan_adv('0_3', pregant_content)
				...
				record.close()
				require('lib/mamashai_db').db.insert_json("last_week_2", "0", e.value)
				logEvent('jiance_month');
		});
		...
		var record = require('/lib/mamashai_db').db.select_one_json("last_week_2", "0")
		if(!record.blank) {
			var last_week = parseInt(record.json)
			if (Ti.App.is_android){
				setTimeout(function(){
					week_container.fireEvent("click", {index: last_week, source: week_container.children[last_week]});
				}, 400);
			}
			else
				week_container.fireEvent("click", {index: last_week, source: week_container.children[last_week]});
