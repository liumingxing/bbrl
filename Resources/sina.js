...
var url = Ti.App.mamashai + "/api/account/baby_calendar_mobile";
if (win.id){
	url += "/" + win.id
}
...
webview.addEventListener('load',function(e) {
    if(!Ti.App.is_android){
	    ...
	    if (info.length > 0)
	    {
			...
			Ti.App.Properties.setString("login_type", "1");		//0:主站, 1:新浪 2:腾讯
			Ti.App.Properties.setString("token", json.token);
			Ti.App.Properties.setString("secret", json.secret);
			Ti.App.Properties.setString("token_type", "sina");
			Ti.App.Properties.setString("expire_at", json.expire_at);
    	...
    	if (e.url.indexOf("blank?")>0){
			...
			Ti.App.Properties.setString("login_type", "1");		//0:主站, 1:新浪 2:腾讯
			Ti.App.Properties.setString("token", params.token);
